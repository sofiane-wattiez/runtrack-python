...
class Can(Canvas):
    ...
    def verificationFinale(self):#Lorsque toutes les cases sont remplies
        ...
        if len(self.cases)==49: # On comptabilise les points
            ...
            if typ[1]==0:
                self.texte2 = Label(fen, text = "Les " + typ[0] + " ont d�finitivement gagn� !", bg= self.fonce,
                                    fg=self.clair, font=self.police1)
                self.texte2.grid()
            elif typ[1]==1:
                self.texte2 = Label(fen, text = "Les " + typ[0] + " ont gagn� les premiers!", bg= self.fonce,
                                    fg=self.clair, font=self.police1)
                self.texte2.grid()
            else:
    ...
    def win(self, qui, p, d): # Partie gagn�e
        ...
        self.qui = qui
        self.texte = Label(fen, text="Les %s ont gagn� !" % (qui), bg= self.fonce, fg=self.clair, font=self.police1)
        self.texte.grid()
        self.running = 0
    ...
    def gagnantes(self, liste=[]): # On v�rifie que les pi�ces ne sont pas encore gagnantes, et on les ajoute dans la liste si elles le deviennent
